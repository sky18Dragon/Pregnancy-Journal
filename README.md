...
- 当前实现切片覆盖幼兔与儿童期主页、成长过场、两个阶段的独立动作和台词、每日结算、持久化和可加速测试时间；蛋的孵化和青年期按后续子功能逐项接入。
...
当前版本从`DAY 1`、`GROWTH 10 / 30`和`LOVE 18`开始。成长值达到30且食物、快乐、精力和清洁度均不低于40时，幼兔会自动播放三段成长过场并进入`CHILD`。儿童期成长目标显示为`GROWTH 30 / 120`，达到120后保留当前儿童期形态，等待青年期子功能接入。
...
9. 重新进入`TEST`面板，连续点击三次`+30 GROWTH`，确认成长值依次达到60、90和120，页面保持`CHILD`，等待后续青年期功能。
10. 断电重启，确认日志显示`save=loaded stage=child`，主页继续显示`CHILD`和断电前的成长、亲密、食物及日期。
11. 在`TEST`面板连续点击两次`RESET PET`，再连续点击三次`NEXT DAY`，确认到`DAY 4`时食物值为20且心情为`HUNGRY`。
12. 此时点击`+30 GROWTH`并退出测试面板，确认成长值达到30但仍为`HATCHLING`，对白提示继续照料以帮助成长。
13. 点击一次`FEED`，确认食物从20恢复到50，并立即播放成长过场；过场结束后进入`CHILD`。这一步验证成长同时需要成长值和良好照料状态。
14. 点击测试面板的`+20 LOVE`把亲密值增加到100，确认继续点击后不会超过100。
15. 返回主页后点击`TALK`，确认高亲密度儿童期台词生效；连续聊天时优先避开最近五句。
16. 再次断电重启，确认儿童期阶段和全部数值继续保留。
