...
- 应用选择窗口打开时按照当前IMU姿态确定画面方向：设备横置使用单行布局，设备竖置使用2×2布局，两个相反放置方向都会保持画面正向；设备平放时沿用当前APP方向。两种布局的整组内容都保持上下留白均衡，四个入口分别显示`Pet`、`Pomodoro Time`、`Status Board`和`Answers of book`，单行与双行文字均按照实际点亮像素在标签框内居中。浅灰网点保留素材层次，当前APP使用黑色标签和闪光标记。
...
- 页面标题为全大写的`STICKY POMODORO TIMER`，顶部使用番茄外形、兔耳、叶冠和兔子五官组成的独立番茄兔像素标志。
...
18. 连续完成三次自定义10秒计时，确认每次`END`都能停止蜂鸣器，并继续保持10秒为选中时间。
19. 分别启动15分钟、25分钟、60分钟和一个自定义时间，确认每一种时长都从开始后的第一秒持续逐秒变化。
