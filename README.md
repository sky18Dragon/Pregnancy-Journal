...
- 点击预设状态后进入二级展示页，状态文字、像素兔子场景和补充信息铺满整个屏幕。
...
2. 确认一级菜单横向显示六个状态，每张卡片都有对应的完整像素兔子场景；默认选中的`IN A MEETING`为黑底白兔线稿，其余状态为白底黑兔线稿。
3. 点击`FOCUSING`，确认进入黑底全屏展示页，大字、专注工作的兔子和`UNTIL 14:30`完整显示。
