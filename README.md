...
各APP页面右上角常驻电池图标和BQ27220提供的0～100%真实剩余电量；外接电源存在时，电池内部显示闪电标记。电量最多每60秒重新读取一次，并随下一次电子纸页面刷新一起更新，避免为了一个缓慢变化的数字额外频繁刷新屏幕。进入深度睡眠时，右上角电池状态由月亮图标接管。
...
5. 同时按住两个侧键进入休眠，确认电池图标被月亮替代；使用AI键唤醒后，电池图标重新出现。
6. 如果电量计暂时读取失败，画面显示`--%`，日志每10秒最多重试一次；恢复读取后应自动显示有效百分比。

成功标准是电量来源为BQ27220、数值保持在0～100%、横竖屏位置一致、外接电源标记正确，并且休眠月亮不会与电池图标重叠。
