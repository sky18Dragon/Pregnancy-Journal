...
当前`feature/ui-experience`分支将番茄钟作为一个完整、独立的APP运行。启动入口直接进入竖屏时间选择主页，用于独立验证时间选择、自定义时间、倒计时、暂停、结束和到时提醒。答案书、状态牌与桌宠APP源码继续保留，等待最终整合。
...
番茄钟APP位于`src/apps/pomodoro/`，当前通过屏幕、触摸和蜂鸣器接口使用硬件。答案书、状态牌、桌宠、姿态与方向页面源码继续保留，后续由产品入口负责选择并启动APP。
