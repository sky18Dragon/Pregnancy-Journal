...
开源素材审计结果、精确版本和许可证保存在`third_party/virtual_pet/`。完整台词源档和规则源档保存在`assets/desktop_pet/library/`；固件只编译筛选后的定长C++数据表，源档不会占用设备Flash或RAM。当前幼兔主页已经接入共享核心的成长、亲密、食物、心情和状态台词；睡眠已经接入测试时间，RTC硬件时间按后续子功能继续接入。
