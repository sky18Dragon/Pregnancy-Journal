...
- 屏幕底部八分之一区域支持向上滑动打开选择窗口；选择窗口中部支持向下滑动关闭。手势按当前横竖画面方向换算，纵向移动至少达到屏幕高度的五分之一时生效。
...
命令正常结束且没有输出，表示横竖屏的底部上滑、中部下滑、起点区域、最短距离、方向、横向漂移和最长持续时间均已覆盖。
...
14. 在横竖两种选择窗口中，从画面中部向下滑动；确认窗口关闭、原APP恢复、IMU停止，并记录`launcher=gesture action=close`和`launcher=cancelled`。
