...
新设备首次启动时先显示八页图文教程，依次介绍桌宠照料与成长、番茄钟、状态板、答案书、APP选择器、睡眠组合键和IMU旋转选择。第一页使用`EXIT`退出，第2～8页使用`BACK`返回上一页，最后一页使用`START`完成教程；完成状态写入独立NVS标记，后续启动直接进入桌宠。宠物蛋和桌宠主页的对话框右侧常驻书本形状的`GUIDE`入口，可在不重启设备的情况下重新打开完整教程。Debug固件的APP选择器右上角仍保留`?`入口，用于清除教程标记并重启复测。
...
- 第一页左下角的`EXIT`结束教程，第2～8页左下角的`BACK`返回上一页，右下角使用`NEXT`前进并在最后一页显示`START`。`EXIT`和`START`都会保存完成标记；重新上电和深度睡眠唤醒后直接进入正常APP流程。
- 宠物蛋和桌宠主页在对话框右侧显示小型书本`GUIDE`入口，图标保持紧凑，实际触摸区域向四周扩展；触摸后暂停桌宠、立即打开教程，退出或完成后恢复原桌宠页面。
...
生成命令会从八张最终设计稿重新生成黑白预览和固件位图。四条测试命令正常结束且没有输出，表示页码边界、退出、返回、完成、八页竖屏位图、底栏热区、宠物蛋自主唤醒隔离和休眠提示音触发策略均通过；页面测试会生成`/tmp/onboarding_1.ppm`至`/tmp/onboarding_8.ppm`用于逐页视觉检查。
