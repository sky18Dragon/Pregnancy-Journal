...
- 当前实现切片覆盖幼兔、儿童期、三种青年期和三种成年期，包含成长过场、分支判定、最终选择页、各路线独立动作与台词、每日结算、持久化和可加速测试时间；蛋的孵化按后续子功能独立接入。
...
当前版本从`DAY 1`、`GROWTH 10 / 30`和`LOVE 18`开始。成长值达到30且食物、快乐、精力和清洁度均不低于40时，幼兔会自动播放三段成长过场并进入`CHILD`。儿童期成长值达到120时，系统比较贪吃、亲密和活力分数；第一名领先第二名至少6分时自动进入对应青年路线，分数接近时由用户在全屏页面完成最后一次选择。青年期成长目标为`GROWTH 120 / 280`，达到280且四项照料状态均不低于40后，会沿着已经确定的性格路线进化为对应成年兔。
...
当前测试存档版本为`4`。已有版本`2`和`3`存档会自动迁移，成长值、亲密值、日期、当天奖励次数和三种性格分数继续保留。青年性格路线和成年阶段都会写入同一份存档，断电重启后继续显示对应形态。
...
pet=ready page=home profile=test save=new stage=hatchling day=1 growth=10 love=18 food=80 mood=happy result=ok
```
...
2. 进入`TEST`面板，连续点击两次`RESET PET`，确认面板显示`HATCHLING`、`DAY 1`、`GROWTH 10`、`LOVE 18`、`FOOD 80`和`HAPPY`。
3. 点击一次`+30 GROWTH`，确认测试面板自动关闭，并依次出现幼兔发光、黑色成长剪影和儿童期揭晓三个全屏画面。
4. 等待过场结束，确认主页标题为`CHILD`，成长目标为`GROWTH 30 / 120`，兔子变得更高、耳朵更长并戴有小围巾。
5. 点击儿童期兔子，确认显示专属抚摸姿势；分别点击`FEED`和`PLAY`，确认显示儿童期独立的吃胡萝卜和离地扑球姿势。
6. 连续点击`TALK`，确认出现儿童期语气的对白，并且成长、亲密和食物值不会因为聊天变化。
7. 停留在儿童期主页约30秒，确认会看到儿童期独立的眨眼、抖耳、转头、伸懒腰、饥饿或疲劳姿势，连续动作不会重复最近两种。
8. 在任意自主动作显示期间点击兔子或底部按钮，确认当前动作会切换为对应用户互动，触摸不会失效。
9. 重新进入`TEST`面板，确认前面三种互动已把成长值累计到110，点击一次`+30 GROWTH`后成长值在120封顶；由于三种互动分数接近，页面自动切换为三选一。
10. 点击`STAY CLOSE`，确认先播放青年进化过场，然后主页标题变为`HEART YOUTH`，成长目标为`GROWTH 120 / 280`，角色带有爱心脸颊和吊坠。
11. 分别点击`FEED`、屏幕中间的兔子、`PLAY`和`TALK`，确认显示亲密型青年兔的四类独立动作和台词。
12. 进入`TEST`面板连续点击六次`+30 GROWTH`，确认成长值在280封顶，并播放青年成长为成年兔的三段全屏过场。
13. 等待过场结束，确认主页标题为`HEART ADULT`、成长值为`GROWTH 280 / 280`，兔子体型、爱心吊坠和拥抱姿态明显升级。
14. 分别点击`FEED`、成年兔身体、`PLAY`和`TALK`，确认显示亲密型成年兔的四类动作和成年专属台词；成长值继续保持280。
15. 断电重启，确认日志显示`save=loaded stage=adult`，主页继续显示`HEART ADULT`和断电前的数值。
16. 验证自动路线：重置后点击一次`+30 GROWTH`进入`CHILD`，在儿童期连续点击两次`FEED`，再在`TEST`面板点击一次`+30 GROWTH`。确认不出现选择页，直接进化为`FOODIE YOUTH`。
17. 在`TEST`面板连续点击两次`RESET PET`，再连续点击三次`NEXT DAY`，确认到`DAY 4`时食物值为20且心情为`HUNGRY`。
18. 此时点击`+30 GROWTH`并退出测试面板，确认成长值达到30但仍为`HATCHLING`，对白提示继续照料以帮助成长。
19. 点击一次`FEED`，确认食物从20恢复到50，并立即播放成长过场；过场结束后进入`CHILD`。这一步验证成长同时需要成长值和良好照料状态。
20. 点击测试面板的`+20 LOVE`把亲密值增加到100，确认继续点击后不会超过100。
21. 返回主页后点击`TALK`，确认高亲密度儿童期台词生效；连续聊天时优先避开最近五句。
22. 再次断电重启，确认儿童期阶段和全部数值继续保留。

成功时，启动日志会出现`pet=ready page=home profile=test`以及当前`stage`；每次照料会出现带`stage`、`food`和`mood`的`pet=care`，测试操作会出现`pet=test`，自动换日会出现带新食物值的`pet=day source=timer`。
