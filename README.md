...
- 应用选择窗口使用四张完整轮廓的兔子像素贴纸，竖屏采用2×2排列，横屏采用单行排列；四个入口分别显示`Pet`、`Pomodoro Time`、`Status Board`和`Answers of book`，单行与双行文字均按照实际点亮像素在标签框内居中。浅灰网点保留素材层次，当前APP使用黑色标签和闪光标记。
...
命令成功后会同时验证八个贴纸触摸点、三个入口间隙、标签文字上下留白以及黑色和浅灰素材层，并生成`/tmp/sticky_launcher_portrait.ppm`和`/tmp/sticky_launcher_landscape.ppm`，分别用于检查竖屏和横屏布局。
