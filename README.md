...
- 支持开始、暂停、继续和提前结束确认。
- 一分钟以上的计时只在分钟节点更新画面。
- 10秒和30秒计时显示开始画面，时间到时直接刷新响铃页面。
- 提前结束后直接返回主页，并恢复默认15分钟。
- 时间到后蜂鸣器持续鸣响，按下`END`立即停止响铃并返回默认15分钟主页。

### 电子纸显示策略

- 页面切换使用黑白全屏刷新。
- 时间选择、数字输入和分钟节点使用黑白局部刷新。
...
9. 再次点击`END SESSION`，然后点击`END NOW`，确认返回默认`15:00`主页。

### 时间到与蜂鸣器

10. 在主页选择10秒并开始计时。
11. 确认10秒内画面保持稳定。
12. 时间到后确认页面显示`TIME'S UP`、`00:00`和`ALARM SOUNDING`，蜂鸣器持续鸣响。
13. 保持10秒不操作，确认蜂鸣器继续鸣响。
14. 点击`END`，确认蜂鸣器立即停止，随后页面返回默认`15:00`主页。
...
18. 连续完成三次10秒计时，确认每次`END`都能停止蜂鸣器并恢复默认15分钟。
