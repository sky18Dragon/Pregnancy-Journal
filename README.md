...
5. 点击一次`PET`，确认数值变为`GROWTH 20 / 30`和`LOVE 28`，兔子闭眼接受抚摸，约1.3秒后恢复待机和默认对白。
6. 再点击一次`PET`，确认数值变为`GROWTH 30 / 30`和`LOVE 38`，只在这一次短暂显示`I'M READY TO GROW!`，随后恢复默认对白。
7. 第三次点击`PET`，确认成长值保持30、亲密值变为48，并显示正常抚摸对白；第四次点击`PET`，确认数值保持不变。
8. 点击`PLAY`，确认兔子以离地扑球姿势出现；由于当天亲密奖励还剩10点，亲密值应变为58，成长值保持30。
9. 点击`FEED`，确认兔子变成抱胡萝卜姿势；当天亲密奖励已经达到40点上限，成长值和亲密值保持不变，但食物值从76恢复到100。
10. 快速连续点击三个入口，确认每次点击都会按顺序处理，页面没有永久卡死，日志依次出现`pet=care action=...`。
