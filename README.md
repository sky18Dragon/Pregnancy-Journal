...
当前`feature/ui-experience`分支将答案书作为一个完整、独立的APP运行。启动入口直接进入竖屏答案书主页，用于独立验证答案类型选择、持续摇晃、思考动画、答案揭晓和再次提问。番茄钟、状态牌与桌宠APP源码继续保留，等待最终整合。
...
答案书APP位于`src/apps/book_of_answers/`，当前通过屏幕、触摸和IMU摇晃接口使用硬件。番茄钟、状态牌、桌宠、姿态与方向页面源码继续保留，后续由产品入口负责选择并启动APP。
