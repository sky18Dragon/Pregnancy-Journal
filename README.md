...
主页顶部右侧依次显示`LOVE`和可点击的`ENERGY`，底部把陪伴天数与RTC日期合并显示为`DAY 4  YYYY-MM-DD`。主页在成长条下方显示`FOOD`和当前心情。初始食物值为80；清醒状态依据RTC真实经过时间降低食物值，`PLAY`还会消耗4点。每次`FEED`都会恢复30点食物值，最高为100，即使当天成长奖励已经用完，喂食仍然有效。食物值不高于30时心情优先显示`HUNGRY`，对白会切换到饥饿语境。
