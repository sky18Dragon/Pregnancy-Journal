...
当前`feature/ui-experience`分支将状态机作为一个完整、独立的APP运行。启动入口直接进入横屏状态选择主页，用于独立验证六种状态、状态详情、自定义输入、返回操作和宠物动画。番茄钟、答案书与桌宠APP源码继续保留，等待最终整合。
...
状态机APP位于`src/apps/status_board/`，当前通过屏幕和触摸接口使用硬件。番茄钟、答案书、桌宠、姿态与方向页面源码继续保留，后续由产品入口负责选择并启动APP。
