...
- 当前实现切片覆盖幼兔、儿童期和三种青年期，包含成长过场、分支判定、最终选择页、各路线独立动作与台词、每日结算、持久化和可加速测试时间；蛋的孵化和成年期按后续子功能逐项接入。
...
当前版本从`DAY 1`、`GROWTH 10 / 30`和`LOVE 18`开始。成长值达到30且食物、快乐、精力和清洁度均不低于40时，幼兔会自动播放三段成长过场并进入`CHILD`。儿童期成长值达到120时，系统比较贪吃、亲密和活力分数；第一名领先第二名至少6分时自动进入对应青年路线，分数接近时由用户在全屏页面完成最后一次选择。青年期成长目标为`GROWTH 120 / 280`。

当前子功能在青年期280点保持已选形态和存档，成年期将在下一个独立阶段接入。
...
成长值会分别在幼兔的30点和儿童期的120点边界停止，亲密值最高为100。每天最多获得100点成长值和40点亲密值。超过当天奖励次数后仍然可以看动作和对白，但数值保持不变。
...
当前测试存档版本为`4`。已有版本`2`和`3`存档会自动迁移，成长值、亲密值、日期、当天奖励次数和三种性格分数继续保留。青年性格路线会在进化动画播放前写入存档，断电重启后继续显示同一形态。
...
命令正常结束且没有输出表示全部断言通过。当前测试覆盖RTC日期、在线与离线时间推进、饥饿衰减、喂食恢复、睡眠、换日、进化、青年自动分支和最终选择、心情、台词防重复、自主动作防重复、动画队列和双槽存档校验。
...
12. 断电重启，确认日志显示`save=loaded stage=youth`，主页继续显示`HEART YOUTH`和断电前的数值。
13. 验证自动路线：重置后点击一次`+30 GROWTH`进入`CHILD`，在儿童期连续点击两次`FEED`，再在`TEST`面板点击一次`+30 GROWTH`。确认不出现选择页，直接进化为`FOODIE YOUTH`。
14. 在`TEST`面板连续点击两次`RESET PET`，再连续点击三次`NEXT DAY`，确认到`DAY 4`时食物值为20且心情为`HUNGRY`。
15. 此时点击`+30 GROWTH`并退出测试面板，确认成长值达到30但仍为`HATCHLING`，对白提示继续照料以帮助成长。
16. 点击一次`FEED`，确认食物从20恢复到50，并立即播放成长过场；过场结束后进入`CHILD`。这一步验证成长同时需要成长值和良好照料状态。
17. 点击测试面板的`+20 LOVE`把亲密值增加到100，确认继续点击后不会超过100。
18. 返回主页后点击`TALK`，确认高亲密度儿童期台词生效；连续聊天时优先避开最近五句。
19. 再次断电重启，确认儿童期阶段和全部数值继续保留。
