...
新存档从全屏宠物蛋开始。轻触蛋三次会依次出现摇摆、第一道裂纹、露出双耳和破壳幼兔；第三次完成后播放一次轻柔上行三音，进入`DAY 1`、`GROWTH 10 / 30`、`LOVE 18`和`FOOD 80`的幼兔主页。每次有效轻触都会立即保存，断电后从已完成的裂纹阶段继续。成长值达到30且食物、快乐、精力和清洁度均不低于40时，幼兔会自动播放三段成长过场并进入`CHILD`。儿童期成长值达到120时，系统比较贪吃、亲密和活力分数；第一名领先第二名至少6分时自动进入对应青年路线，分数接近时由用户在全屏页面完成最后一次选择。青年期成长目标为`GROWTH 120 / 280`，达到280且四项照料状态均不低于40后，会沿着已经确定的性格路线进化为对应成年兔。
