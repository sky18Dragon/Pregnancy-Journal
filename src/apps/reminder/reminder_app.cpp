...
bool read_now() {
  ...
  if (sticky_rtc_read(clock) != ESP_OK ||
      !sticky_rtc_epoch_seconds(clock, s_now))
    return false;
...
size_t visible(const Reminder *items[kVisibleRows]) {
  if (s_today_mode) {
    ...
    const size_t count =
        s_service.get_today(s_today, mutable_items, kVisibleRows);
    for (size_t i = 0U; i < count; ++i)
      items[i] = mutable_items[i];
    return count;
  }
...
void render_list() {
  ...
  s_canvas->draw_text(34, 76, ui_text(s_today_mode ? "TODAY" : "UPCOMING"), 2,
                      GrayLevel::DarkGray);
  button(540, 24, 226, 48,
         s_service.any_enabled() ? "ALL REMINDERS ON" : "ALL REMINDERS OFF",
         s_service.any_enabled());
  const Reminder *items[kVisibleRows] = {};
  const size_t count = visible(items);
  if (count == 0U) {
    s_canvas->draw_text(54, 190, ui_text("NO REMINDERS"), 4, GrayLevel::Black);
...
void render_add() {
  ...
  const char *types[] = {"GENERAL", "SUPPLEMENT", "CHECKUP",
                         "WATER",   "ACTIVITY",   "REST"};
...
void handle_press(int x, int y) {
  ...
  if (x >= 540 && y >= 24 && y < 72) {
    s_service.set_all_enabled(!s_service.any_enabled());
    reminder_storage_save(s_service);
    render();
    return;
  }
...
uint32_t reminder_app_power_sleep_timeout_ms() {
  return s_adding ? 0U : kSleepTimeoutMs;
}
