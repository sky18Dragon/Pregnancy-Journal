...
bool reminder_valid(const Reminder &reminder) {
  return reminder.schema_version == kReminderSchemaVersion &&
         ...
         reminder.title[0] != '\0' && reminder.type <= ReminderType::Rest &&
...
bool ReminderService::create(Reminder reminder, uint32_t *created_id) {
  ...
  if (!reminder_valid(reminder) || find(reminder.id) != nullptr)
    return false;
  if (reminder.id >= next_id_)
    next_id_ = reminder.id + 1U;
...
bool ReminderService::update(const Reminder &reminder) {
  ...
  if (current == nullptr || !reminder_valid(reminder))
    return false;
  *current = reminder;
  return true;
}
...
bool ReminderService::complete(uint32_t id) {
  ...
  if (!pregnancy_date_to_day_index(reminder->date, day) ||
      !pregnancy_date_from_day_index(day + increment, next_date))
    return false;
...
bool ReminderService::toggle(uint32_t id) {
  Reminder *reminder = find(id);
  if (reminder == nullptr)
    return false;
...
void ReminderService::set_all_enabled(bool enabled) {
  for (size_t index = 0U; index < count_; ++index)
    reminders_[index].enabled = enabled;
}

bool ReminderService::any_enabled() const {
  for (size_t index = 0U; index < count_; ++index)
    if (reminders_[index].enabled)
      return true;
  return false;
}
...
size_t ReminderService::get_upcoming(uint32_t now, const Reminder *output[],
  ...
  for (size_t index = 0U; index < count_ && found < capacity; ++index) {
    uint32_t epoch = 0U;
    if (reminders_[index].enabled && !reminders_[index].completed &&
        reminder_epoch(reminders_[index], epoch) && epoch > now)
      output[found++] = &reminders_[index];
  }
...
const Reminder *ReminderService::next(uint32_t now,
                                      uint32_t *epoch_seconds) const {
  ...
  for (size_t index = 0U; index < count_; ++index) {
    ...
    if (!reminders_[index].enabled || reminders_[index].completed ||
        !reminder_epoch(reminders_[index], epoch) || epoch <= now ||
        (best != nullptr && epoch >= best_epoch))
      continue;
    best = &reminders_[index];
    best_epoch = epoch;
  }
  if (best != nullptr && epoch_seconds != nullptr)
    *epoch_seconds = best_epoch;
  return best;
}
