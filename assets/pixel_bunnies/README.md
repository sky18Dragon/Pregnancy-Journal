...
- `out_for_lunch_alt_v2`：兔子、桌椅和餐盘保持固定，持勺前臂将勺子抬到嘴边。
- `off_duty_alt_v2`：兔子上半身和背包保持固定，前后腿交换形成连续迈步。
...
脚本会更新80×80菜单预览、160×160子页动画预览和`src/ui/assets/status_bunny_assets.cpp`。生成时会校验用餐和下班动画两帧的主体尺寸与中心位置，确保动作发生在固定场景中。固件采用每像素1位的高位优先排列，80×80素材每张占800字节，160×160素材每张占3200字节。
