...
The current `source/` set contains the approved room, Hatchling and Child identity art, plus three distinct Youth personality forms. Every Youth form has its own idle, signature movement, feed, pet, and play poses. The `firmware/` directory contains the generated 1-bit previews, subject masks, four action icons, and love icon.
